...
static void read_watch_descriptors_count() {
  ...
  // NOTE: currently WSL does not have any limit on maximum number of watches
  // so it does not have /proc/sys/fs/inotify/max_user_watches file.
  // to work around it, we just set a hard number here
  watch_count = 524288;
}
